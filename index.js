...
export function autoDetectBlindOpenings(image, canvasSlotList = []) {
    return new Promise((resolve, reject) => {
        ...
        // If OpenCV is initialized, perform the detection immediately
        if (cv.getBuildInformation) {
          performDetection();  // Call detection logic directly if already initialized
        } else {
          // Wait for OpenCV to be fully initialized
          cv['onRuntimeInitialized'] = () => {
            performDetection();  // Call detection logic after initialization
          };
        }
        ...
        function performDetection() {
            try {
                ...
                reject('Error during detection: ' + error); // Reject the Promise if there's an error
            }
        }
    });
};